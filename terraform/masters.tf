resource "azurerm_network_interface" "master-nic" {
  ...
  name                = "master-nic${count.index - (floor(count.index / var.master_count) * var.master_count)}"
...
resource "azurerm_virtual_machine" "master" {
  ...
  name                  = "master${count.index - (floor(count.index / var.master_count) * var.master_count)}"
  location              = "${element(azurerm_resource_group.rg.*.location, floor(count.index / var.master_count))}"
  resource_group_name   = "${element(azurerm_resource_group.rg.*.name, floor(count.index / var.master_count))}"
  ...
  storage_os_disk {
    name              = "master-osdisk${count.index -  (floor(count.index / var.master_count) * var.master_count)}"
  ...
  os_profile {
    computer_name  = "${element(azurerm_resource_group.rg.*.name, floor(count.index / var.master_count))}-master${count.index - (floor(count.index / var.master_count) * var.master_count)}"

resource "azurerm_network_interface" "agent-nic2" {
  ...
  name                = "${azurerm_resource_group.rg.2.name}-agent-nic${count.index}"
...
resource "azurerm_virtual_machine" "agent2" {
  ...
  name                  = "${azurerm_resource_group.rg.2.name}-agent${count.index}"
  location              = "${azurerm_resource_group.rg.2.location}"
  resource_group_name   = "${azurerm_resource_group.rg.2.name}"
  ...
  storage_os_disk {
    name              = "${azurerm_resource_group.rg.2.name}-agent-osdisk${count.index}"
  ...
  os_profile {
    computer_name  = "${azurerm_resource_group.rg.2.name}-agent${count.index}"

resource "azurerm_network_interface" "agent-nic4" {
  ...
  name                = "${element(azurerm_resource_group.rg.*.name, 4)}-agent-nic${count.index}"
  ...
  ip_configuration {
    ...
    private_ip_address            = "${cidrhost(10.4.0.0/16, count.index+5+var.master_count)}"
  }
...
resource "azurerm_virtual_machine" "agent4" {
  ...
  name                  = "${element(azurerm_resource_group.rg.*.name, 4)}-agent${count.index}"
  location              = "${element(azurerm_resource_group.rg.*.location, 4)}"
  resource_group_name   = "${element(azurerm_resource_group.rg.*.name, 4)}"
  ...
  storage_os_disk {
    name              = "${element(azurerm_resource_group.rg.*.name, 4)}-agent-osdisk${count.index}"
  ...
  os_profile {
    computer_name  = "${element(azurerm_resource_group.rg.*.name, 4)}-agent${count.index}"
